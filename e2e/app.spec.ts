...
test.describe('real playable loop', () => {
  test('desktop: auto combat → Pyra → loot → equip → stronger → reload → defeat → retry', async ({ page }) => {
    ...
    await expect.poll(async () => (await debug<Snapshot>(page, 'snapshot')).enemyHp, { timeout: 5000 }).toBeLessThan(start.enemyHp);
    await expect.poll(async () => (await debug<Snapshot>(page, 'snapshot')).pyraCharge, { timeout: 5000 }).toBeGreaterThan(0);
  ...
  test('desktop: x2 and x3 increase authoritative simulation rate', async ({ page }) => {
    await page.goto('/'); await logicalClick(page, 640, 608); const a = await page.evaluate(() => (window as any).__FA_DEBUG__.state().statistics.totalSimulationMs); await page.waitForTimeout(500); const b = await page.evaluate(() => (window as any).__FA_DEBUG__.state().statistics.totalSimulationMs); const x2Delta = b - a;
    await logicalClick(page, 732, 608); const c = await page.evaluate(() => (window as any).__FA_DEBUG__.state().statistics.totalSimulationMs); await page.waitForTimeout(500); const d = await page.evaluate(() => (window as any).__FA_DEBUG__.state().statistics.totalSimulationMs); const x3Delta = d - c;
    expect(x2Delta).toBeGreaterThan(650); expect(x3Delta).toBeGreaterThan(x2Delta * 1.2); expect(await page.evaluate(() => (window as any).__FA_DEBUG__.state().settings.speed)).toBe(3);
  });
  ...
  test('phone landscape: touch speed control and automatic combat work', async ({ page }, testInfo) => {
    ...
    await expect.poll(async () => (await debug<Snapshot>(page, 'snapshot')).enemyHp, { timeout: 6000 }).toBeLessThan(before.enemyHp); await expect.poll(async () => (await debug<Snapshot>(page, 'snapshot')).pyraCharge).toBeGreaterThan(0); expect(await page.evaluate(() => document.documentElement.scrollWidth > innerWidth || document.documentElement.scrollHeight > innerHeight)).toBe(false); expect(errors).toEqual([]);
  });
});
