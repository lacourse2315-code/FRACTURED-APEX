export interface DevWorld { id: string; name: string; minStage: number; maxStage: number }
export interface DevEnemy { id: string; name: string; worldId: string; power: number }
export interface GameContent { worlds: DevWorld[]; enemies: DevEnemy[] }
...
export function validateContent(content: GameContent): string[] {
  ...
  for (const world of content.worlds) {
    ...
    if (ids.has(world.id)) errors.push(`Duplicate ID: ${world.id}`); ids.add(world.id);
  ...
  for (const enemy of content.enemies) {
    ...
    if (ids.has(enemy.id)) errors.push(`Duplicate ID: ${enemy.id}`); ids.add(enemy.id);
