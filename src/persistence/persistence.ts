...
function validItem(value: unknown): value is EquipmentItem {
  ...
  return (
    ...
    Number(item.itemLevel) >= 1 &&
    !!item.primary &&
    typeof item.primary === 'object' &&
...
export function isValidState(value: unknown): value is GameState {
  ...
  if (
    !s.campaign ||
    s.campaign.worldId !== 'dev-fracture' ||
    !Number.isInteger(s.campaign.stage) ||
    s.campaign.stage < 1 ||
    !Number.isInteger(s.campaign.highestCleared) ||
    s.campaign.highestCleared < 0
  )
    return false;
  ...
  if (
    !s.equipped ||
    slots.some(
      (slot) => !(slot in s.equipped!) || (s.equipped![slot] !== null && typeof s.equipped![slot] !== 'string'),
    )
  )
    return false;
  ...
  if (
    !s.statistics ||
    !nonNegative(s.statistics.totalSimulationMs) ||
    !nonNegative(s.statistics.victories) ||
    !nonNegative(s.statistics.defeats)
  )
    return false;
  return true;
}
...
function migrateV1(value: Record<string, unknown>): GameState | null {
  ...
  if (settings && [1, 2, 3].includes(Number(settings.speed)))
    fresh.settings.speed = Number(settings.speed) as 1 | 2 | 3;
