...
declare global { interface Window { __FA_DEBUG__?: DebugApi } }
...
export class DevScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.add.text(w / 2, 18, STRINGS.title, { fontFamily: 'Arial', fontSize: '30px', fontStyle: 'bold', color: '#ffffff' }).setOrigin(0.5, 0);
    this.add.text(w / 2, 52, STRINGS.subtitle, { fontFamily: 'Arial', fontSize: '12px', color: '#8fa9d8' }).setOrigin(0.5, 0);
    this.stageText = this.add.text(24, 20, '', { fontFamily: 'Arial', fontSize: '16px', color: '#a9c7ff' });
    this.powerText = this.add.text(w - 24, 20, '', { fontFamily: 'Arial', fontSize: '16px', align: 'right', color: '#ffd778' }).setOrigin(1, 0);
    this.makeMute(w); this.makeActors(w, h); this.makeHud(w, h); this.makeSpeedButtons(w, h); this.makeNav(w, h); this.makeResultPanel(w, h); this.updateUi();
    this.scale.on('resize', () => this.scene.restart());
    if (import.meta.env.DEV) window.__FA_DEBUG__ = { snapshot: () => this.core.snapshot(), state: () => this.state, power: () => this.core.power(), loot: () => this.core.loot() };
  }
  ...
  private drawBackdrop(w: number, h: number): void {
    const g = this.add.graphics(); g.fillGradientStyle(0x05071b, 0x171044, 0x050817, 0x260d43, 1); g.fillRect(0, 0, w, h);
    for (let i = 0; i < 58; i++) { g.fillStyle(0xffffff, 0.15 + (i % 4) * 0.06); g.fillCircle((i * 101) % w, (i * 59) % h, 1 + (i % 2)); }
    g.lineStyle(2, 0x7a43ff, 0.2); g.strokeEllipse(w / 2, h * 0.43, w * 0.42, h * 0.5);
  }

  private makeActors(w: number, h: number): void {
    this.actor(w * 0.23, h * 0.47, 'RIFTWARDEN', 0x54b9ff); this.actor(w * 0.41, h * 0.53, 'PYRA', 0xff8f4a, 28); this.actor(w * 0.76, h * 0.47, 'ENEMY', 0xff577d);
    this.heroWeapon = this.add.rectangle(w * 0.29, h * 0.46, 76, 10, 0xb9e5ff).setOrigin(0, 0.5).setRotation(-0.22);
  }
  private actor(x: number, y: number, label: string, color: number, radius = 42): void {
    const g = this.add.graphics(); g.fillStyle(color, 0.92); g.fillCircle(x, y, radius); g.lineStyle(3, 0xffffff, 0.65); g.strokeCircle(x, y, radius);
    this.add.text(x, y + radius + 12, label, { fontFamily: 'Arial', fontSize: '13px', color: '#ffffff' }).setOrigin(0.5, 0);
  }
  private makeHud(w: number, h: number): void {
    this.heroHp = this.add.text(24, h * 0.24, '', { fontFamily: 'Arial', fontSize: '16px', color: '#8fffc1' });
    this.enemyHp = this.add.text(w - 24, h * 0.24, '', { fontFamily: 'Arial', fontSize: '16px', color: '#ff9caf', align: 'right' }).setOrigin(1, 0);
  ...
  private makeMute(w: number): void {
    ...
    bindTap(mute, () => { this.state.settings.muted = !this.state.settings.muted; this.sound.mute = this.state.settings.muted; mute.setText(this.state.settings.muted ? 'UNMUTE' : STRINGS.mute); this.onSave(this.state); }); this.sound.mute = this.state.settings.muted;
  }
  private makeSpeedButtons(w: number, h: number): void {
    ([1, 2, 3] as GameSpeed[]).forEach((speed, i) => {
      const t = this.add.text(w / 2 - 92 + i * 92, h - 112, `x${speed}`, { fontFamily: 'Arial', fontSize: '18px', backgroundColor: this.state.settings.speed === speed ? '#5841a8' : '#1b2343', padding: { x: 20, y: 10 }, color: '#fff' }).setName(`speed-${speed}`);
      bindTap(t, () => { this.state.settings.speed = speed; this.sim.setSpeed(speed); this.onSave(this.state); this.scene.restart(); });
    });
  }
  ...
  private makeResultPanel(w: number, h: number): void {
    this.resultText = this.add.text(w / 2, h * 0.18, '', { fontFamily: 'Arial', fontSize: '24px', fontStyle: 'bold', color: '#fff', backgroundColor: '#0c122ddd', padding: { x: 18, y: 10 } }).setOrigin(0.5).setDepth(20);
    this.lootText = this.add.text(w / 2, h * 0.31, '', { fontFamily: 'Arial', fontSize: '14px', align: 'center', color: '#eef3ff', backgroundColor: '#101731ee', padding: { x: 18, y: 14 }, wordWrap: { width: 430 } }).setOrigin(0.5, 0).setDepth(20);
    this.diagnosticText = this.add.text(w / 2, h * 0.31, '', { fontFamily: 'Arial', fontSize: '17px', color: '#ffcf8a', backgroundColor: '#101731ee', padding: { x: 18, y: 12 } }).setOrigin(0.5).setDepth(20);
    ...
    const hero = this.actionButton(w / 2 + 65, h * 0.58, 'HERO', () => this.showHeroPanel(w, h)); hero.setName('defeat-hero');
  }
  private actionButton(x: number, y: number, label: string, action: () => void): Phaser.GameObjects.Text {
    const t = this.add.text(x, y, label, { fontFamily: 'Arial', fontSize: '14px', color: '#fff', backgroundColor: '#5841a8', padding: { x: 15, y: 10 } }).setOrigin(0.5).setDepth(21).setName(`action-${label.toLowerCase()}`); bindTap(t, action); return t;
  }
  private showHeroPanel(w: number, h: number): void {
    const stats = this.core.stats();
    const gear = Object.entries(this.state.equipped).map(([slot, id]) => `${slot.toUpperCase()}: ${id ? this.state.inventory.find((i) => i.instanceId === id)?.name ?? 'Unknown' : 'Empty'}`).join('\n');
    const t = this.add.text(w / 2, h * 0.22, `RIFTWARDEN\nPOWER ${this.core.power()}\nHP ${stats.maxHp}  ATK ${stats.attack}  DEF ${stats.defense}\nCRIT ${(stats.critChance * 100).toFixed(1)}%  CDR ${(stats.cooldownRecovery * 100).toFixed(1)}%\n\n${gear}\n\nTap to close`, { fontFamily: 'Arial', fontSize: '15px', color: '#eef3ff', align: 'left', backgroundColor: '#0c122df4', padding: { x: 22, y: 18 } }).setOrigin(0.5, 0).setDepth(30); bindTap(t, () => t.destroy());
  }
  ...
  private formatLoot(item: EquipmentItem): string {
    ...
    const delta = this.core.comparison(); const locked = this.state.lockedItemIds.includes(item.instanceId) ? 'LOCKED • ' : '';
  ...
  private presentEvent(event: ReturnType<PlayableCore['tick']>[number]): void {
    if (event.type === 'damage' && event.amount) this.floater(event.target === 'hero' ? 310 : 965, 310, `${Math.round(event.amount)}${event.critical ? '!' : ''}`, event.target === 'hero' ? '#ff8a8a' : '#ffffff');
    if (event.type === 'shield' && event.amount) this.floater(310, 350, `SHIELD ${Math.round(event.amount)}`, '#83c9ff'); if (event.type === 'pyra' && event.label) this.floater(520, 350, event.label, '#ffad67');
  }
  private floater(x: number, y: number, text: string, color: string): void {
    if (this.floaters.length >= 12) this.floaters.shift()?.destroy(); const t = this.add.text(x, y, text, { fontFamily: 'Arial', fontSize: '17px', fontStyle: 'bold', color }).setOrigin(0.5).setDepth(15); this.floaters.push(t);
    this.tweens.add({ targets: t, y: y - 36, alpha: 0, duration: 700, onComplete: () => { this.floaters = this.floaters.filter((f) => f !== t); t.destroy(); } });
  }
  private toast(message: string, w: number, h: number): void { const t = this.add.text(w / 2, h * 0.72, message, { fontFamily: 'Arial', fontSize: '15px', backgroundColor: '#10172e', padding: { x: 16, y: 10 }, color: '#ffdb8a' }).setOrigin(0.5).setDepth(25); this.time.delayedCall(1200, () => t.destroy()); }
}
